 ...
 *
 * @apiSuccess (204) – No content (book deleted)
 *
...
deleteBookByISBNRouter.delete(
  ...
  (req: ISBNRequest, res: Response, next: NextFunction) => {
    ...
    res.status(400).send({ message: 'Invalid ISBN - must be a 13-digit string' });
  },
  ...
  async (req: ISBNRequest, res: Response) => {
    ...
    try {
      ...
      await client.query('DELETE FROM author_books WHERE isbn13 = $1', [req.cleanedISBN13]);
      await client.query('DELETE FROM book_ratings WHERE isbn13 = $1', [req.cleanedISBN13]);

      const { rowCount } = await client.query('DELETE FROM books WHERE isbn13 = $1', [req.cleanedISBN13]);
